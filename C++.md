...
- **const**
  ...
  4. const和类：①const修饰成员变量，在某个对象的声明周期内是常量，但是对于整个类而言是可以改变的。因为类可以创建多个对象，不同的对象其const成员变量的值是不同的。切记，**不能在类内初始化const成员变量**（C++11之后可以），因为类的对象没创建前，编译器并不知道const成员变量是什么，因此const数据成员只能在初始化列表中初始化。②const修饰成员函数，主要目的是防止成员函数修改成员变量的值，即该成员函数并不能修改成员变量。③const对象，常对象，常对象只能调用常函数。
...
动态初始化也叫运行时初始化。主要是指需要经过函数调用才能完成的初始化或者是类的初始化，一般来说是局部静态类对象的初始化和局部静态变量的初始化。**对于全局或者类的静态成员变量，是在main()函数执行前由运行时调用相应的代码进行初始化的。**而对于局部静态变量，是在函数执行至此初始化语句时才开始执行的初始化。[(86条消息) C++中静态变量的初始化时间_c++静态成员变量什么时候初始化__Santiago的博客-CSDN博客](https://blog.csdn.net/mary288267/article/details/121162529)
...
局部静态变量位于内存中的静态存储区，未经初始化的局部静态变量会自动初始化为0。但是局部静态变量的作用于还是还是局部作用域，定义它的函数或者代码块结束的时候，作用于也随之结束。但是该变量值不会被销毁，而是在内存中驻留下来，知道程序全部结束，这个驻留的值我们不能访问她。
...
T&& t在发生自动类型推断的时候，它是未定的引用类型（universal references），如果被一个左值初始化，它就是一个左值；如果它被一个右值初始化，它就是一个右值，它是左值还是右值取决于它的初始化。无论传入的形参是左值还是右值，对于函数模板内部来说，形参既有名称又能寻址，因此它都是左值。**在函数模板中的右值引用又被称为万能引用。**
...
**但当数据成员中有指针时，会出问题。如果没有自定义拷贝构造函数，会调用默认拷贝构造函数，这样就会调用两次析构函数。**第一次析构函数delete了内存，第二次的就指针悬挂了。所以，此时，必须采用深拷贝。**一个带有堆内存的类，必须提供一个深拷贝拷贝构造函数**。
...
- **那些函数不能是虚函数？**
  ...
  ⑤**成员函数模板不能是虚函数**。因为c++ 编译器在解析一个类的时候就要确定虚函数表的大小，如果允许一个虚函数是模板函数，那么compiler就需要在parse这个类之前扫描所有的代码，找出这个模板成员函数的调用（实例化），然后才能确定vtable的大小，而显然这是不可行的，除非改变当前compiler的工作机制。编译阶段，编译器要实例化所有模板。基类看不到子类的虚函数模板到底有哪些实例。虚函数的多态性是通过虚函数表和虚函数指针实现的，而成员模板函数的调用在编译时期是无法确定的，因为模板函数的具体实现是在实例化时才确定的，因此无法在编译时期生成虚函数表和虚函数指针。

  **模板类可以有虚函数**，因为模板类在实例化时会生成具体的类，这些生成的类会包含实际的成员函数。这些成员函数可以被声明为虚函数，从而允许在派生类中进行重写，实现多态性。对于模板类而言，实际的类类型在编译时才会确定，因此虚函数的调用仍然可以在运行时进行动态绑定，实现多态性的效果。
...
- 只能在堆上生成对象的类。
  ...
  只能在堆上也就意味着不能再栈上，在栈上是编译器分配内存空间，构造函数来构造栈对象。在栈上当对象周期完成，编译器会调用析构函数来释放栈对象所占的空间，也就是说编译器管理了对象的整个生命周期。**编译器在调用构造函数为类的对象分配栈空间时，会先检查析构函数的访问性**，不光是析构函数，编译器会检查所有非静态函数的访问性。因此，如果类的析构函数是私有的，编译器不会为对象在栈上分配内存空间。
  ...
  如果将构造函数也设为[private](https://so.csdn.net/so/search?q=private&spm=1001.2101.3001.7020)或protected, 但需要让该类提供一个static成员函数专门用于产生该类型的堆对象. （设计模式中的singleton模式就可以用这种方式实现. ）让我们来看看:
  ...
  class  NoStackObject
  	...
  	int main()
  	{
  		//NoStackObject *a=new NoStackObject;无法访问类protected成员
...
- 只能在栈上生成对象的类。
  ...
  只有使用new运算符才会在堆上创建对象。将operator new设为私有即可，保证在类外面无法定义，也无法使用。

  动态建立类对象，是使用new运算符将对象建立在堆空间中。这个过程分为两步，第一步是执行operator new()函数，在堆空间中搜索合适的内存并进行分配；第二步是调用构造函数构造对象，初始化这片内存空间（这种方法，间接调用类的构造函数），但是operator new()函数用于分配内存，无法提供构造功能，**所以将构造函数必须为public**。
...
在C++中，对象的构造过程包括了对象的内存分配、初始化和构造函数的调用。当使用构造函数创建一个对象时，编译器在内存中为对象分配了足够的空间，并在这段内存中调用构造函数来进行对象的初始化。在构造函数内部，对象已经被分配了内存空间，但还没有完全构造完成，包括对象的成员变量也还没有被初始化。

C++的构造函数在被调用时，隐式地传递了一个指向当前对象的指针，称为 this 指针。这个 this 指针指向了对象在内存中的地址，因此在构造函数内部就可以使用 this 指针来引用对象的成员变量和成员函数，进行对象的初始化操作。

这是因为在构造函数中，对象的内存空间已经被分配，但对象的初始化过程还没有完全结束。使用 this 指针可以让构造函数在对象初始化期间对对象的成员进行操作，例如给成员变量赋值、调用成员函数等。一旦构造函数执行完毕，对象的初始化过程就完成了，对象就可以在程序中被正常使用了。

需要注意的是，在构造函数中使用 this 指针时，要注意不要引用尚未初始化的成员变量，以避免出现未定义的行为。同时，要注意构造函数的执行顺序，包括成员变量的初始化顺序和基类构造函数的调用顺序，以确保对象在构造过程中得到正确的初始化。

**默认构造函数：**如果没有人为构造函数，则编译器会自动默认生成一个无参构造函数。

**一般构造函数：**包含各种参数，一个类可以有多个一般构造函数，前提是参数的个数和类型和传入参数的顺序都不相同，根据传入参数调用对应的构造函数。

**拷贝构造函数：**拷⻉构造函数的函数参数为对象本身的引用，用于根据⼀个已存在的对象复制出⼀个新的该类的对象，⼀般在函数中会将已存在的对象的数据成员的值⼀⼀复制到新创建的对象中。如果没有**显示的写拷⻉构造函数，则系统会默认创建⼀个拷⻉构造函数**，但当类中有指针成员时，最好不要使⽤编译器提供的默认的拷⻉构造函 数，最好⾃⼰定义并且在函数中执⾏深拷⻉。
...
**类型转换构造函数：**有时候不想要隐式转换，用explict关键字修饰。一般来说带一个参数的构造函数，或者除第一个参数外其他参数是默认的构造函数。      



## 什么情况下会调用拷贝构造函数？

1. 对象以值传递的方式进入函数体  fun(obj)
2. 对象以值传递的方式从函数返回   return obj
3. 一个对象需要另外一个对象初始化  obj1 = obj2
...
1. 成员变量在使用初始化列表初始化时，只与**定义成员变量的顺序有关**，与构造函数中初始化成员列表的顺序无关。因为成员变量的初始化次序是根据变量在内存中次序有关，而内存中的排列顺序早在编译期就根据变量的定义次序决定了。
2. 如果不使用初始化列表初始化，在构造函数内初始化时，此时与成员变量在构造函数中的位置有关。
3. 类成员在定义时，是不能初始化的(C++11之前)
4. 类中const成员常量必须在构造函数初始化列表中初始化（C++11之前）。
5. 类中static成员变量，必须在类外初始化。（C++11后如果其被声明为 constexpr 或者 const 整型类型，并且可以在编译时求值，那么可以在类内直接初始化。）
...
Derived(int value1, int value2) : Base(value1) {        // 派生类构造函数的实现    }

如果派生类没有使用成员列表初始化语法，则将使用默认的基类构造函数，如果基类没有默认的构造函数就会报错。第二个原因：因为即使继承了，它的名字和派生类的名字也不一样（构造函数名字和类名一样），不能成为派生类的构造函数，当然更不能成为普通的成员函数。在设计派生类时，对继承过来的成员变量的初始化工作也要由派生类的构造函数完成，但是大部分基类都有private属性的成员变量，它们在派生类中无法访问，更不能使用派生类的构造函数来初始化。这种矛盾在C++继承中是普遍存在的，解决这个问题的思路是：**在派生类的构造函数中调用基类的构造函数**。
...
既然new/delete 的功能完全覆盖了malloc/free，为什么C++不把malloc/free 淘汰出局呢？这是因为C++程序经常要调用C 函数，而C 程序只能用malloc/free 管理动态内存。如果用free 释放“new 创建的动态对象”，那么该对象因无法执行析构函数而可能导致程序出错。如果用delete 释放“malloc 申请的动态内存”，理论上讲程序不会出错，但是该程序的可读性很差。所以new/delete 必须配对使用，malloc/free 也一样。
...
- **new[]和delete[]**
  ...
  如果new和delete[]使用，delete的时候会找前4个字节，看要释放的内存有多大。但是使用new导致没有开辟区部分4个字节用来存放数组长度，这样会导致这4个字节是未定义的，因此会调用不定次的delete。同时比如说其实地址为A，应该从A开始释放，现在会从A-4开始释放。

  如果new[]和delete一起使用，程序会认为这是一个对象占用的空间，而不是数组，因此就析构一次。同时释放的是new[]中表示长度的前4个字节的地址，应该从A-4开始释放，如果不从头释放的话会出问题。
...
2. **dynamic_cast**
   ...
   pa 是`A*`类型的指针，当 pa 指向 A 类型的对象时，向下转型失败，pa 不能转换为`B*`或`C*`类型。当 pa 指向 D 类型的对象时，向下转型成功，pa 可以转换为`B*`或`C*`类型。

   ![img](http://c.biancheng.net/uploads/allimg/190218/1525153Y0-0.jpg)
...
4. **reinterpret_cast**
   ...
   reinterpret的意思是重新解释，可以将**任意类型指针转换为其他类型的指针**。所以他的type-id必须是一个指针、引用、算术类型。
...
- **auto**
  ...
  auto自动帮我们获得定义变量的类型所以auto定义的变量**必须有初始值**。

  auto可以推断基本类型，也可以推断引用类型，当推断引用类型时候，将引用对象的类型作为推断类型（重要）

  auto的自动类型推断发生在编译期，所以使用auto并不会造成程序运行时效率的降低。

  编译器可以根据初始值自动推导出类型。但是不能用于函数传参、数组类型的推导、类的非静态成员变量的初始化、作为模板参数。

  ```
  //当auto变量不是指针或者引用类型时，推导的结果中不会保留 const、volatile 关键字
  //当auto变量是指针或者引用类型时，推导的结果中会保留 const、volatile 关键字
  ```
...
- **decltype**
  ...
  **decltype 的考虑是：参数如果是变量名，就返回其声明的类型；而参数是表达式，再根据表达式的值类型来判断是否保留引用。 decltype(x) 是变量名的规则，而 decltype((x)) 是表达式的规则，井水不犯河水。**

  表达式为普通变量或者普通表达式或者类表达式，在这种情况下，使用 decltype 推导出的类型和表达式的类型是一致的。

  表达式是函数调用，使用 decltype 推导出的类型和函数返回值一致。（只用类类型保有const，volatile）
...
1. const修饰的成员变量。因为const只能被初始化不能被赋值（C++11前）
...
RVO (return value optimization) 和NRVO (named return value optimization) 是C++在处理 “返回一个class object的函数” 时常用的优化技术，主要作用就是**消除临时对象的构造和析构成本**。

RVO（Return Value Optimization，返回值优化）和 NRVO（Named Return Value Optimization，命名返回值优化）是 C++ 编译器进行的优化技术，用于在函数返回值的处理中避免不必要的对象复制或移动操作，从而提高程序性能。

RVO 是指在函数返回时，将局部对象直接构造到调用者提供的对象的内存空间中，从而避免了额外的对象拷贝或移动。NRVO 则是一种特殊的 RVO，它在函数中使用了命名的返回值（即通过返回值声明的变量），并且该变量在函数体内只被用于作为返回值，没有被其他操作修改过，编译器可以直接将局部对象构造到该命名返回值的内存空间中，也避免了额外的对象拷贝或移动。

RVO 和 NRVO 可以有效减少函数返回值的拷贝或移动操作，从而提高程序性能，尤其是在涉及到大型对象或者代价较高的构造函数时。这些优化是由编译器在编译阶段进行的，对于程序员而言是透明的，不需要显式的代码改变。然而，不同的编译器和编译器选项可能会对 RVO 和 NRVO 的优化行为有所不同，因此在对性能要求较高的代码中，建议进行性能测试和分析，以确保获得期望的优化效果。
...
在使用 `std::move` 返回值时，实际上是将右值引用类型的对象转移给接收变量。这意味着接收变量可以是右值引用类型，也可以是左值引用类型，甚至可以是普通的值类型。例如，在下面的示例中：

```
MyClass func() {
    MyClass obj;
    // ...
    return std::move(obj); // 返回一个右值引用
}
MyClass obj2 = func(); // 使用右值引用类型的变量接收函数返回值
```

`obj2` 是一个普通的值类型变量，而不是右值引用类型变量。这是因为 C++ 中有一个右值引用到值类型的转移规则，即当右值引用的对象被赋值给一个新的变量时，会调用移动构造函数来转移对象的所有权。这意味着 `obj2` 将获得 `func()` 返回值的所有权，而 `func()` 中的原始对象将处于有效但未定义的状态。

当您使用`std::move`函数将局部对象`obj`转换为右值时，它会返回一个右值引用。但是，当您将这个右值引用作为函数的返回值时，编译器会将其视为一个右值，并尝试使用移动构造函数来创建返回值。

需要注意的是，在这种情况下，使用`std::move`函数并不会带来任何好处。事实上，它可能会阻止编译器进行返回值优化（RVO，Return Value Optimization），导致程序运行效率降低。因此，在函数返回局部对象时，通常不建议使用`std::move`函数。

需要注意的是，虽然可以将 `func()` 返回值赋值给一个普通值类型的变量，但这并不是推荐的做法。在实际编程中，为了避免潜在的错误，通常建议使用右值引用类型的变量来接收 `std::move` 返回值，以明确表达对对象所有权的转移。例如：

```
MyClass&& obj2 = func(); // 使用右值引用类型的变量接收函数返回值
```

这样可以更明确地表达代码意图，避免可能的误解。然而，由于右值引用类型的变量在赋值后可能处于未定义状态，因此需要小心处理，确保在之后不再访问该变量。

1. **当用户没有显式定义任何构造函数（包括移动构造函数和拷贝构造函数）时，编译器会自动创建默认构造函数、移动构造函数和拷贝构造函数。这被称为合成的移动构造函数（Synthesized Move Constructor）。**
2. **当用户显式定义了拷贝构造函数时，编译器不会自动生成移动构造函数。在这种情况下，如果需要使用移动构造函数，需要显式地定义移动构造函数。**

合成的移动构造函数会自动执行浅拷贝操作，将成员变量的值从源对象移动到目标对象，并将源对象的成员变量置为有效但未定义的状态。这种自动生成的移动构造函数通常足够满足大多数情况下的需求。

需要注意的是，如果类的成员变量有指针或资源管理的情况，可能需要显式定义移动构造函数来进行深拷贝或资源的转移。此外，如果类中有显式定义的拷贝构造函数、析构函数或赋值运算符重载函数时，编译器不会自动生成合成的移动构造函数，需要显式定义移动构造函数来支持移动语义。
...
您的程序只输出了一次“copy”，这可能是由于您使用的编译器版本或设置不同导致的。不同版本的编译器对`-fno-elide-constructors`选项的处理可能会有所不同。没有优化应该是1次默认2次copy/move。

1.在`fun`函数返回时，返回的是一个局部对象`a`。根据C++标准，当函数返回一个局部对象时，编译器会将其视为右值（rvalue），并尝试使用移动构造函数来创建临时对象。这样可以避免不必要的复制操作，提高程序的运行效率。

在您的代码中，由于定义了移动构造函数，所以编译器会使用移动构造函数来创建临时对象。如果您没有定义移动构造函数，编译器会使用复制构造函数来创建临时对象。

2.在`main`函数中，将返回的临时对象赋值给名为`x`的`A`类对象时，编译器会尝试使用移动构造函数来创建`x`对象。这是因为临时对象是一个右值（rvalue），移动构造函数可以避免不必要的复制操作，提高程序的运行效率。

3.`std::move`函数可以将一个左值（lvalue）转换为右值（rvalue）。它通常用于在不需要复制对象的情况下，将对象的所有权从一个变量转移到另一个变量。在您提供的代码中，`fun`函数返回时，编译器会自动将返回的局部对象`a`视为右值。因此，您不需要使用`std::move`函数来转换它。

但是，在其他情况下，您可能需要使用`std::move`函数来显式地将一个左值转换为右值。例如，当您想要将一个容器的内容移动到另一个容器中时，您可以使用`std::move`函数来避免不必要的复制操作。
...
容器开辟的时候还没有值，无法初始化，你咋能用引用。那么当你写个vector<int&> ivec(30),这个时候你怎么初始化
...
如果需要空间动态缩小，可以考虑使用deque。如果vector，可以用swap()来帮助你释放内存。直接用vec.shrink_to_fit()
...
vec.push_back(2);
// ...
vec.swap(std::vector<int>()); // 通过与一个临时的vector对象进行交换，释放vec的内存空间
```
...
![image-20230419165422861](C:\Users\admin\AppData\Roaming\Typora\typora-user-images\image-20230419165422861.png)
...
list<int>::iterator it(rit.base()); //变为正向迭代器
cout << *rit << endl; 
cout << *it << endl;
```
...
关联式容器 `std::map` 和 `std::set` 内部存储的是键值对 (key-value) 或者单一的键 (key)，而不仅仅是元素本身。每个元素都包含了键和值（对于 `std::map`），或者只有键（对于 `std::set`），以及与红黑树相关的节点信息，如指向父节点、左子节点、右子节点的指针等。这样的内部结构使得关联式容器的内存布局不是连续存储的，而是通过树节点之间的链接来组织的。

因此，关联式容器 `std::map` 和 `std::set` 无法像 `std::vector` 一样通过预分配连续的内存空间来提高性能，因为其内部节点信息的存储方式和动态分配的特性使得预分配内存空间的方式无法直接适用。关联式容器的插入和删除操作通常需要重新平衡红黑树的结构，而不仅仅是移动元素，因此无法像 `std::vector` 那样简单地进行内存复制和移动操作。
...
- 迭代器的类型
  ...
  6. 此外，C++11和之后的标准还引入了一些额外的迭代器类型，如常量迭代器（Const Iterator）、反向迭代器（Reverse Iterator）和移动迭代器（Move Iterator）等，用于提供更多的迭代器功能和灵活性。
